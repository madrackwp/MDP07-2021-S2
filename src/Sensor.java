...
public class Sensor {
	...
	private static final int WIDTH = 15; // Not sure what this is
	private static final int HEIGHT = 20;
	int range; // Set the range of the sensor
	SensorLocation currentDirection;
	int locationOnRobot_x; // location of sensor
	int locationOnRobot_y;
	int robot_x; // location of robot
	int robot_y;
	...
	// When robot turns left, it has to update the sensor locations
	...
	// Updating the location of the robot
	public void updateRobotLocation(int x, int y) {
		robot_x = x;
		robot_y = y;
	}

	// Look at the robot's current location and update the cells around it
	...
	public boolean Sense(Map map, int data, int[][] notWorkinghe) {
		...
		for (int i = 1; i < range + 1; i++) {
			...
			// hitwill will be true when senselocation returns a true which indicates that a
			// wall has been met
			if (!hitWall) {
