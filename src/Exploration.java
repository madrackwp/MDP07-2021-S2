...
public class Exploration {
	...
	// Constructor for the Exploration model
	public Exploration(SocketClient sc, boolean simulator, RobotInterface robot, Visualization viz, Map inMap) {
		...
		state = ExplorationState.INITIAL_EXPLORATION; // Telling the algorithm what type of state the robot it is in

		// init the list to all have "NO.ACTION"
		listOfActions = new Action[listOfActionsSize]; // Action is an enum
	...
	boolean ismapCoordinateInStack(int[] mapCoordinate) {
		...
		for (int i = 0; i < unexploredAreas.size(); i++) {
			...
			if (unexplored[0] == mapCoordinate[0] && unexplored[1] == mapCoordinate[1] && unexplored[2] == mapCoordinate[2]
		...
		for (int y = 0; y < Map.HEIGHT; y++) {
			for (int x = 0; x < Map.WIDTH; x++) {
				if (mapArray[y][x] == ExplorationTypes.toInt("UNEXPLORED_EMPTY")
					...
					// need to input a area that the robot has explored and is next to the
		...
		if (goingToBlock) {
			...
			if (unexploredAreas.empty()) {
				...
				if (!unexploredAreas.empty()) {
					...
					pathToNextUnexploredArea = new Astar(map.getNodeXY(robot.x, robot.y),
							map.getNodeXY(unexploredAreas.get(indexOfClosestArea)[2], unexploredAreas.get(indexOfClosestArea)[3]));

					// System.out.print("next area to explore " +
					// unexploredAreas.get(indexOfClosestArea)[0] +" and "+
					// unexploredAreas.get(indexOfClosestArea)[1] +
					// " but going to " + unexploredAreas.get(indexOfClosestArea)[2] + " and " +
			...
			while (true) {
				...
				switch (state) {
					case INITIAL_EXPLORATION:
						...
						// once it reaches the start point, function will return true and go on to next
						// state
						if (DoInitialExploration()) {
							...
							// remove blocks when there are more than 30 blocks and change all unexplored
							// areas to explored
						...
						// Astar pathToStart = new Astar(map.getNodeXY(robot.x, robot.y),
					...
					case CLEARING_UNKNOWN:
						...
						map.updateMap();
						// calculate the fastest path back home
						// PathDrawer.update(robot.x, robot.y,
						// pathToNextUnexploredArea.getFastestPath());
