...
public class Exploration {
	...
	//bool to check weather the robot has moved, to prevent the exploration ending immediately cause since
	//robot starts at the start position and i use it to check if exploration is done
	boolean robotMoved;

	//prevent robot from front calibrating twice
	boolean hasJustFrontCalibrated;
	//the stack to contain the unexplored areas of the map, arrays will contain {x,y}
	Stack<int[]> unexploredAreas;

	//the Astar class to the next unexplored area
	Astar pathToNextUnexploredArea;

	//get the next unexplored area to go to, coordinates will be {x of grid,y of grid, x of grid that i will go to "look" at target block, y of looking grid}
	...
	int getMapExplored()
	{
		...
		for (int i = 0; i < Map.HEIGHT; i++)
			for (int j = 0; j < Map.WIDTH; j++)
				if (map.getMapArray()[i][j] == ExplorationTypes.toInt("EMPTY") || map.getMapArray()[i][j] == ExplorationTypes.toInt("OBSTACLE"))
					explored++;
	...
	boolean ismapCoordinateInStack(int[] mapCoordinate)
	{
		...
		for(int i = 0; i < unexploredAreas.size(); i++)
		{
			...
			if(unexplored[0] == mapCoordinate[0] &&
				unexplored[1] == mapCoordinate[1] &&
				unexplored[2] == mapCoordinate[2] &&
				unexplored[3] == mapCoordinate[3])
				return true;
		}
		return false;
	}
	...
	void updateUnexploredAreas()
	{
		...
		for(int i = 0; i < unexploredAreas.size(); i++)
		{
			//if the area is now not a "unexplored", means it was explored on the way, then remove it from the stack
			if(mapArray[unexploredAreas.get(i)[1]][unexploredAreas.get(i)[0]] == ExplorationTypes.toInt("EMPTY") ||
					mapArray[unexploredAreas.get(i)[1]][unexploredAreas.get(i)[0]] == ExplorationTypes.toInt("OBSTACLE"))
			{
				//System.out.print("removed" + unexploredAreas.get(i)[0] + " , " + unexploredAreas.get(i)[1] + " with index " + i + "\n");
	...
	public int calculateNumBackTrackSteps()
	{
		...
		//return the number of steps to take before being next to a wall to continue the algo
		return numSteps;
	}
	void doStoredActions()
	{
		//robot will do the actions that have been stored here instead of doing computation in StartExploration()
	...
	public boolean DoInitialExploration()
	{
		...
		if(traceBackSteps.size() > 75)
		{
			//if(map.mapScoreArray[13][12] > 22)
				map.turnoffgrid = true;
				map.turnoffgrid3 = true;
	...
	{
		//count how many obstacles there are, if there are above 30, remove the ones with the lowest score
		...
		while(obstacleAndScore.size() > 30)
		{
			...
			map.mapScoreArray[obstacleAndScore.get(lowestScoreIndex)[1]][obstacleAndScore.get(lowestScoreIndex)[0]] = -1;
