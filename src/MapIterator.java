...
public class MapIterator {
        ...
        try {
            ...
            for (int w = mapDescriptorP1.length-1; w >= 0 ; w--) {
                for (int h = 0; h < mapDescriptorP1[0].length ; h ++) {                   
                    if(results[w][h] == ExplorationTypes.toInt("EMPTY") || results[w][h] == ExplorationTypes.toInt("OBSTACLE"))
                    {
                    	sb.append(1);
            ...
            for (int w = mapDescriptorP1.length-1; w >= 0 ; w--) {
                for (int h = 0; h < mapDescriptorP1[0].length ; h ++) { 
                    ...
                    if(mapDescriptorP1[w][h] == 1)
                    {
						...
						//if(w==0 && h==mapDescriptorP1[0].length-1) { sb.append(0); sb.append(0); hexSB.append(0); hexSB.append(0); }
    ...
    public static String formatStringToHexadecimal(String string) {
        ...
        try {
            ...
            if (decimal < 16) {
                // if it is less than 16, the hexadecimal produced will only be one digit instead of 2
                stringBuilder.append("0");
            }
            hexStr = Integer.toString(decimal,16);
            stringBuilder.append(hexStr);
