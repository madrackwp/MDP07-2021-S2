...
public final class PathDrawer {
	...
	public static void drawPath(Graphics g)
	{
        ...
        g.drawLine(30+robotX*Map.sizeofsquare,30+robotY*Map.sizeofsquare,
      			30+path.get(path.size()-1).getX()*Map.sizeofsquare,30+path.get(path.size()-1).getY()*Map.sizeofsquare);
	...
	{
		if(unexploredAreas == null || unexploredAreas.empty()) return;
		...
		{
			//g.drawRect(10+(unexploredAreas.get(i)[0]-1)*Map.sizeofsquare, 10+(unexploredAreas.get(i)[1]-1)*Map.sizeofsquare, 40, 40);
			g.fillRect(10+(unexploredAreas.get(i)[0])*Map.sizeofsquare, 10+(unexploredAreas.get(i)[1])*Map.sizeofsquare, 38, 38);
		}
	}
}